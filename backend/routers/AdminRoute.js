...
// get leave requests
router.get('/leave/all', (req, res) => {
    const sql = `
      SELECT lr.id, lr.reason, lr.status, lr.created_at, e.name 
    ...
    con.query(sql, (err, result) => {
      if (err) {
        ...
        return res.json({ success: false, message: "Failed to fetch leaves" });
      }
...
//   update leave requests
  router.put('/leave/update/:id', (req, res) => {
    ...
    con.query(sql, [status, id], (err, result) => {
      ...
      return res.json({ success: true, message: "Leave status updated" });
    });
  });
